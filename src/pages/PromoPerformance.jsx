...
function DailySales({ country, onLoaded }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true); setError(null)
    try {
      const params = new URLSearchParams({ mode: 'daily_sales', date: selectedDate })
      ...
      {totalsLoaded && years.length > 0 && (
        <div className="flex flex-wrap gap-3 mb-6">
          {[...years].reverse().map((yr, i) => {
            ...
            return (
              <div key={yr} className="flex-1 min-w-[160px] border border-gray-200 rounded-xl p-4 bg-white">
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-3 h-3 rounded-full" style={{background: color}}/>
                  <span className="text-sm font-bold text-gray-700">{yr}</span>
                </div>
                <p className="text-2xl font-black text-gray-900 tabular-nums leading-none">
                  {fmtUsd(k.totalGmv)}
                </p>
                <p className="text-xs text-gray-500 mt-1">{(k.totalOrders||0).toLocaleString()} órdenes</p>
                {k.peakGmv > 0 && (
      ...
      {totalsLoaded && years.length > 0 && (
        <div className="mt-6 overflow-x-auto">
          <table className="w-full text-xs text-right border-collapse">
            ...
            <tbody>
              {chartData.map(row => {
                const hasAny = years.some(yr => row[yr] != null && row[yr] > 0)
                return (
                  <tr key={row.hour} className={`border-b border-gray-100 ${hasAny ? '' : 'opacity-40'}`}>
                    ...
                    {[...years].reverse().map((yr, i) => (
                      <td key={yr} className="py-1.5 px-3 tabular-nums"
                          style={{color: row[yr] > 0 ? YEAR_COLORS[i] : '#9ca3af'}}>
